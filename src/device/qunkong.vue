    ...
    <Container style="flex-direction: column;justify-content: space-between;height: 100%;">
      <Header style="background-color: #DCDFE6;padding: 0;margin: 0;">
        <Menu mode="horizontal">
          ...
          <MenuItem>
            <Button size="mini" @click="$store.state.SelectDevice=[]">全取消</Button>
          </MenuItem>
          ...
          <MenuItem>
            <ElSwitch v-model="$store.state.log" inactive-text="日志"></ElSwitch>
          </MenuItem>
          <MenuItem>
            <router-link to="/">
          ...
          <MenuItem>
            <router-link to="/task">
              <Button
                size="mini"
                @click="$store.commit('createTask',true )"
                :type=" $store.state.isCreateTask ? 'danger' :'' "
              >添加任务</Button>
            </router-link>
          </MenuItem>
